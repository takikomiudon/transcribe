...
無音待機中は5秒ごとに接続維持用の100ms無音だけを送り、発話開始時には直前500msの音声を付け、16kHz PCMを100ms単位で送信します。1.5秒の無音または30秒の連続音声で発話を確定します。周囲の雑音で区切られない場合は、`--silence-threshold 800` のように既定値500より大きくしてください。
